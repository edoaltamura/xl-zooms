...
def make_param_file_gadget4(dir, fname, boxsize, starting_z, finishing_z,
    ...
    #a = 1./ (1+2.8)
    #rlabel = '"%s"'%fname
    #r = [rlabel, fname, '%.8f'%starting_a, '%.8f'%finishing_a, '%.8f'%boxsize,
    #    '%.8f'%omega0, '%.8f'%omegaL, '%.8f'%omegaB, '%.8f'%h,
    #    '%.8f'%s_high, '%.8f'%s_high, '%.8f'%s_low,
    #    '%.8f'%s_low_low, '%.8f'%s_high, '%.8f'%s_high, '%.8f'%(s_high*a), '%.8f'%(s_high*a),
    #    '%.8f'%(s_low*a), '%.8f'%(s_low_low*a), '%.8f'%(s_high*a), '%.8f'%(s_high*a),
    #    '%i'%OutputListOn, OutputListFilename, TimeBetSnapshot, TimeOfFirstSnapshot,
    #    sf_comment, sf_comment]
