...
class read_swift:
    ...
    def select_region(self, part_type, x_min, x_max, y_min, y_max, z_min, z_max, eps=1e-4, just_load_all=False):
        ...
        if 'Cells' in f:
            ...
            mask = np.logical_and(centres[:,0] + half_size[0] >= coords[0] - dx_over_2,
                np.logical_and(centres[:,0] - half_size[0] <= coords[0] + dx_over_2,
                np.logical_and(centres[:,1] + half_size[1] >= coords[1] - dy_over_2,
                np.logical_and(centres[:,1] - half_size[1] <= coords[1] + dy_over_2,
                np.logical_and(centres[:,2] + half_size[2] >= coords[2] - dz_over_2,
    ...
    def read_dataset(self, parttype, att, physical_cgs=False):
        ...
        # Convert?
        if physical_cgs:
            fac = f['PartType%i/%s'%(parttype, att)].attrs.get("Conversion factor to physical CGS (including cosmological corrections)")
            return_array *= fac
...
if __name__ == '__main__':
    snap = '/cosma7/data/dp004/rttw52/swift_runs/runs/Sibelius_Candidate_24943_1213/5Mpc/Sibelius_24943_1213_5Mpc_64/snapshots/Sibelius_24943_1213_5Mpc_64_0002.hdf5'
    x = read_swift(snap)
    x.select_region(1,0,1000,0,1000,0,1000)
    x = x.read_dataset(1, 'Coordinates')
    print(x)
