...
bh_mass = np.empty(num_snaps)
star_mass = np.empty(num_snaps)
...
for i, snap_number in enumerate(snaps_collection[::-1]):
    ...
    try:
        ...
        star_mass[i] = vr_data.apertures.mass_star_100_kpc[0].to(unyt.msun)
        bh_mass[i] = (vr_data.apertures.sfr_gas_100_kpc[0]).to(sfr_output_units)
        delete_last_line()
    except:
        redshifts[i] = np.nan
        sfr[i] = np.nan
