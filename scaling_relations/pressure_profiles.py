...
class PressureProfiles(HaloProperty):
    ...
    def process_single_halo(
        ...
        pressure = (5. / 3. - 1) * sw_data.gas.internal_energies[index] * sw_data.gas.densities[index]
        pressure.convert_to_units('keV/cm**3')

        pressure_profile = histogram_unyt(
            radial_distance,
            bins=lbins,
            weights=pressure,
            normalizer=weighting
        )
        pressure_profile.convert_to_units('keV/cm**3')
        ...
        P500 = (
                500 * fb * kBT500 * critical_density / (mp * mean_molecular_weight)
        ).to('keV/cm**3')

        return radial_bin_centres, pressure_profile, P500
